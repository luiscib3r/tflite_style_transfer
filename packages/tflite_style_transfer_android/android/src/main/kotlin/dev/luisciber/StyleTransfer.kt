...
class StyleTransfer(
    ...
    init {
        if (useGPU) {
            interpreterPredict = getInterpreter(STYLE_PREDICT_FLOAT16_MODEL, true)
            interpreterTransform = getInterpreter(STYLE_TRANSFER_FLOAT16_MODEL, true)
        } else {
            interpreterPredict = getInterpreter(STYLE_PREDICT_INT8_MODEL, false)
            interpreterTransform = getInterpreter(STYLE_TRANSFER_INT8_MODEL, false)
        }
    }
    ...
    private fun getInterpreter(
        ...
        tfliteOptions.numThreads = numberThreads
        return Interpreter(loadModelFile(modelName), tfliteOptions)
    }
    ...
    fun runStyleTransfer(
        ...
        try {
            ...
            val styleBitmap =
                ImageUtils.decodeBitmap(File(styleImagePath))
            val input =
    ...
    companion object {
        fun newCPUStyleTransfer(context: Context): StyleTransfer {
            return StyleTransfer(context, false)
        }

        fun newGPUStyleTransfer(context: Context): StyleTransfer {
            return StyleTransfer(context, true)
        }

        private const val numberThreads = 4
        public const val TAG = "StyleTransfer"
