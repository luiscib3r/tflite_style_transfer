...
class StyleTransfer {
    ...
    static func newCPUStyleTransferer(
        completion: @escaping ((Result<StyleTransfer>) -> Void)
    ) -> () {
        return StyleTransfer.newInstance(
            transferModel: Constants.Int8.transferModel,
    ...
    static func newGPUStyleTransferer(
        ...
        return StyleTransfer.newInstance(
            transferModel: Constants.Float16.transferModel,
    ...
    static func newInstance(
        ...
        let tfLiteQueue = DispatchQueue(label: "dev.luisciber.style_transfer_tflite")
        
        // Run initialization in background thread to avoid UI freeze.
        tfLiteQueue.async {
            // Construct de path to the model file
            guard
            ...
            do {
                ...
                let styleTransfer = StyleTransfer(
                ...
                DispatchQueue.main.async {
                    completion(.success(styleTransfer))
                }
            } catch let error {
    ...
    private func postprocessImageData(
        data: Data,
        size: CGSize = Constants.inputImageSize
    ) -> CGImage? {
